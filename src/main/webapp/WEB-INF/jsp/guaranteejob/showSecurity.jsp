	...
	<table cellpadding="10" id="at-position-table" class="table_list">
		...
		<tr>
			...
			<td><select id="cc_edit" class="easyui-combobox"
				name="details_page_category" style="width: 250px;" required disabled>
					<option value="普通"
						<c:if test="${data.detailsPageCategory=='普通'}">selected="selected"</c:if>>普通</option>
					<option value="担保"
						<c:if test="${data.detailsPageCategory=='担保'}">selected="selected"</c:if>>担保</option>
			</select></td>
			...
			<td><select id="cc_guarantee" class="easyui-combobox"
				name="whether_on_homepage_guarantee" style="width: 250px;" required disabled>
					<option value="是"
						<c:if test="${data.whetherOnHomepageGuarantee=='是'}">selected="selected"</c:if>>是</option>
					<option value="否"
						<c:if test="${data.whetherOnHomepageGuarantee=='否'}">selected="selected"</c:if>>否</option>
			</select></td>
		</tr>
		<tr>
			...
			<td><input class="easyui-datetimebox" name="good_begin_time"
				data-options="required:true,showSeconds:false"
				value="${data.goodBeginTime}" style="width: 250px" required disabled></td>
			<td style="text-align: right">首页担保结束日期:</td>
			<td><input class="easyui-datetimebox" name="good_end_time"
				data-options="required:true,showSeconds:false"
				value="${data.goodEndTime}" style="width: 250px" required disabled></td>
		</tr>
		...
		<tr>
			...
			<td><select id="cc_Recommend" class="easyui-combobox"
				name="whether_on_homepage_recommend" style="width: 250px;" required disabled>
					<option value="是"
						<c:if test="${data.whetherOnHomepageRecommend=='是'}">selected="selected"</c:if>>是</option>
					<option value="否"
						<c:if test="${data.whetherOnHomepageRecommend=='否'}">selected="selected"</c:if>>否</option>
			</select></td>
			<td style="text-align: right">推荐开始日期:</td>
			<td><input class="easyui-datetimebox"
				name="recommend_begin_time"
				data-options="required:true,showSeconds:false"
				value="${data.recommendBeginTime}" style="width: 250px" required disabled></td>
		</tr>
		<tr>
			<td style="text-align: right">推荐结束日期:</td>
			<td><input class="easyui-datetimebox" name="recommend_end_time"
				data-options="required:true,showSeconds:false"
				value="${data.recommendEndTime}" style="width: 250px" required disabled></td>
			<td style="text-align: right">工作日期:</td>
			<td><input type="text" name="work_date" class="easyui-textbox"
				value="${data.workDate}" style="width: 250px" required disabled/></td>
		</tr>

		<tr>
			<td style="text-align: right">工作时间:</td>
			<td><input type="text" name="work_time" class="easyui-textbox"
				value="${data.workTime}" style="width: 250px" required disabled/></td>
			<td style="text-align: right">工作地点:</td>
			<td><input type="text" name="work_location" class="easyui-textbox"
				value="${data.workLocation}" style="width: 250px" required disabled/></td>
		</tr>
		<%-- <tr>
		...
		<tr>
			...
			<td><input type="text" name="work_content"
				value="${data.workContent }" class="easyui-textbox" style="width: 250px"
			...
			<td><select id="cc_Settlement" class="easyui-combobox"
				name="settlement_method" style="width: 250px;" required disabled>
					<option value="日结"
						<c:if test="${data.settlementMethod=='日结'}">selected="selected"</c:if>>日结</option>
					<option value="周结"
						<c:if test="${data.settlementMethod=='周结'}">selected="selected"</c:if>>周结</option>
					<option value="月结"
						<c:if test="${data.settlementMethod=='月结'}">selected="selected"</c:if>>月结</option>
					<option value="完工结算"
						<c:if test="${data.settlementMethod=='完工结算'}">selected="selected"</c:if>>完工结算</option>
					<option value="平台支付随时提现"
						<c:if test="${data.settlementMethod=='平台支付随时提现'}">selected="selected"</c:if>>平台支付随时提现</option>
			</select></td>
		</tr>
		<tr>
			<td style="text-align: right">工资待遇:</td>
			<td><input type="text" name="salary_treatment"
				value="${data.salaryTreatment}" class="easyui-textbox"
				style="width: 250px" required disabled/></td>
		...
		<tr>
			...
			<td><input type="text" name="other_requirements" class="easyui-textbox"
				style="width: 250px" value="${data.otherRequirements}" required disabled/></td>
		...
		<tr>
			...
			<td><input type="text" name="people_num"
				value="${data.peopleNum }" class="easyui-textbox" style="width: 250px"
				required disabled/></td>
			<td style="text-align: right">招聘到期时间:</td>
			<td><input class="easyui-datetimebox"
				name="hiring_expiration_date"
				data-options="required:true,showSeconds:false"
				value="${data.hiringExpirationDate}" style="width: 250px" required disabled></td>
		</tr>
		<tr>
			...
			<td><select id="cc_recruit" class="easyui-combobox"
				name="recruitment_status" style="width: 250px;" required disabled>
					<option value="进行中"
						<c:if test="${data.recruitmentStatus=='进行中'}">selected="selected"</c:if>>进行中</option>
					<option value="已经暂停"
						<c:if test="${data.recruitmentStatus=='已经暂停'}">selected="selected"</c:if>>已经暂停</option>
					<option value="已经结束"
						<c:if test="${data.recruitmentStatus=='已经结束'}">selected="selected"</c:if>>已经结束</option>
					<option value="已经删除"
						<c:if test="${data.recruitmentStatus=='已经删除'}">selected="selected"</c:if>>已经删除</option>
					<option value="已经被投诉下架"
						<c:if test="${data.recruitmentStatus=='已经被投诉下架'}">selected="selected"</c:if>>已经被投诉下架</option>
			</select></td>
			<td style="text-align: right">浏览次数:</td>
			<td><input type="text" name="view_count"
				value="${data.viewCount}" class="easyui-textbox" style="width: 250px"
				required disabled/></td>
		</tr>
		<tr>
			<td style="text-align: right">收藏次数:</td>
			<td><input type="text" name="collection_count"
				value="${data.collectionCount}" class="easyui-textbox"
				style="width: 250px" required disabled/></td>
		...
		<tr>
			...
			<td><input type="text" name="main_title"
				value="${data.mainTitle }" class="easyui-textbox" style="width: 250px"
				required disabled/></td>
		...
		<tr>
			...
			<td><input type="text" name="short_description"
				value="${data.shortDescription}" class="easyui-textbox"
				style="width: 250px" required disabled/></td>
			...
			<td><%-- <input type="file" name="page_url" value="${data.page_url}"
			    ...
			    <a href="TP/${data.pageUrl}" target="_blank" ><img alt="封页图URL" src="TP/${data.pageUrl}" width=250px height=150px></a>
				</td>
		</tr>
		<tr>
			...
			<td><%-- <input type="file" name="first_page_carousel_url"
				...
				<a href="TP/${data.firstPageCarouselUrl}" target="_blank" ><img alt="首页图URL" src="TP/${data.firstPageCarouselUrl}" width=250px height=150px></a>
				</td>	
			...
			<td><%-- <input type="file" name="main_page_url"
				...
				required /> --%>
				<a href="TP/${data.mainPageUrl}" target="_blank" ><img alt="详情页主图URL" src="TP/${data.mainPageUrl}" width=250px height=150px></a>
				</td>
		</tr>
		<tr>
			<td style="text-align: right">商户名称:</td>
			<td><input type="text" name="merchants_name"
				value="${data.merchantsName }" class="easyui-textbox"
				style="width: 250px" required disabled/></td>
		...
		<td colspan="3"><input type="text" name="details_page_introduction"
			value="${data.detailsPageIntroduction }" class="easyui-textbox"
