...
<%
	...
	String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()
			+ path + "/";
%>
<table id="hzn_homepage_guarantee"></table>
<script>
$.Admin.hznHomepageGuarantee = {
    //组件ID
    'id' : '#hzn_homepage_guarantee',
    //工具栏
    'tools' : [     
        { text: '是否上首页担保', iconCls: 'fa fa-edit', handler: function(){
                $.Admin.hznHomepageGuarantee.UpOrDown();
            } 
        },
        { text: '关闭', iconCls: 'fa fa-edit', handler: function(){
            $.Admin.hznHomepageGuarantee.Down();
        } 
   		},
        { text: '详情', iconCls: 'fa fa-edit', handler: function(){
            $.Admin.hznHomepageGuarantee.showSecurity();
            }
        },
        { 
          text:'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input id="hzn_home_guarantee_search" style="width:200px">', 
        },
        {
          iconCls:'icon-search',
          handler:function(){ 
          $.Admin.hznHomepageGuarantee.search();
        }
        },
        {
    ...
    //模糊查询
    'search' :function(){
    	var a=$('#hzn_home_guarantee_search').val();
    	var search={};
    	search.search=a;
          $.ajax({
        	  type:'post',
              ...
              success:function(search_list){
            	  $('#hzn_homepage_guarantee').datagrid('loadData', search_list);
              }
    ...
    //是否上首页担保
    'UpOrDown' : function(){
        var get_select_row = $($.Admin.hznHomepageGuarantee.id).datagrid('getSelected');
        ...
        $(account_id).dialog({
            ...
            iconCls: 'fa fa-edit',
            queryParams: data,
            href: "homepageGuarantee/showWeatherHomepageGuarantee",
            modal: true,
            width: 400,
            onClose : function(){
                $(this).dialog("destroy");
  ...
  //关闭上首页担保
    'Down' : function(){
        ...
        $(account_id).dialog({
            ...
            iconCls: 'fa fa-edit',
            queryParams: data,
            href: "homepageGuarantee/showDowmHomepageGuarantee",
            modal: true,
            width: 400,
            onClose : function(){
                $(this).dialog("destroy");
...
$($.Admin.hznHomepageGuarantee.id).datagrid({
	title: '当前位置：业务管理 > 担保职位管理> 是否上首页担保',
    border: false,
    toolbar: $.Admin.hznHomepageGuarantee.tools,
    fitColumns: true,
    fit: true,
    ctrlSelect: true,
    singleSelect: false,
...
/* 	          {field:'release_time',title:'发布时间',align:'center',width:100},
	...
	function showImage3(value,row,index){
     ...
     $('#hzn_home_guarantee_search').textbox({ icons: [{ 
    	 iconCls: 'icon-clear', 
    	 handler: function (e) { 
